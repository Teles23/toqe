...
| `/(barbeiro)/clientes`             | Lista textual com SortButton azul                          | `SortButton` migrado para tokens (`surface`/`borderStrong`/`captionBold`) — visualmente coerente com o resto da app. SearchInput e ClienteCard ja herdam os tokens novos. Avatares com `Avatar` (iniciais hash-coloridas) ja vinha do DS base                                                                                                                                                                                                                                                                                                     | ✅ Commit 9 |
| `/(barbeiro)/perfil`               | Form com 7 sub-rotas usando Button shim azul               | Index com `DangerButton` "Sair"; sub-rotas (2fa, senha, editar, sessoes, notificacoes) migradas para `AmberButton`/`GhostButton`/`DangerButton` conforme papel da CTA. `SecaoCard`/`ListItem` ja vinham com tokens novos                                                                                                                                                                                                                                                                                                                          | ✅ Commit 9 |
