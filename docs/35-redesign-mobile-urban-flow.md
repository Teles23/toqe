...
| `/(auth)/cadastro`                 | Form único de 5 campos, sistema                            | Brand tag âmbar em pill no topo, título "Criar conta" em Sora, 5 campos com FormInput dark, AmberButton "Criar conta", footer link "Entrar" em âmbar. **GAP 3:** agora em **3 steps visuais** com `StepIndicator` (pontos âmbar + conectores). Step 1: email/senha/confirmar. Step 2: nome + telefone. Step 3: `TipoContaToggle` Cliente                                                                                                                                                                                                          | Barbeiro + AmberButton "Criar conta". Em 409, volta ao step 1 marcando o email | ✅ Commit 4 |
...
| `/(barbeiro)/fila`                 | Lista de cards com nome e hora                             | **Live progress bar** por tempo de espera (`useInterval` 30s + `getWaitTone`/`getWaitProgress` puros) com cor verde→âmbar→vermelho; posição na fila em mono XL âmbar; status badge com dim background; long-press → BottomSheet. 9 testes unitários do util de espera + 6 do card + 5 da tela. **Refino futuro:** drag-to-reorder para o barbeiro reorganizar a fila                                                                                                                                                                              | ✅ Commit 7                                                                    |
| `/(barbeiro)/clientes`             | Lista textual com SortButton azul                          | `SortButton` migrado para tokens (`surface`/`borderStrong`/`captionBold`) — visualmente coerente com o resto da app. SearchInput e ClienteCard ja herdam os tokens novos. **GAP 1:** novo `ClienteAvatar` com iniciais hash-color em 5 cores Urban Flow Dim (âmbar/verde/teal/vinho/azul-escuro), determinístico por nome. Identificador secundário no card: **telefone em `typography.mono`** (dígitos alinhados), fallback para email se ausente. `getInitialsColor`/`getInitials` puros com 12 testes                                          | ✅ Commit 9                                                                    |
| `/(barbeiro)/perfil`               | Form com 7 sub-rotas usando Button shim azul               | Sub-rotas migradas para `AmberButton`/`GhostButton`/`DangerButton`. **GAP 2:** header com `ClienteAvatar` size=80 hash-color centralizado, nome em `typography.title` Sora, role label em caption muted ("Barbeiro"/"Cliente"/"Dono"/"Admin"). Seções reorganizadas (Barbearia → Conta → Segurança → Preferências); email vira ListItem readonly em "Conta" (sai do header). `DangerButton` "Sair da conta" no rodapé                                                                                                                             | ✅ Commit 9                                                                    |
| `/(cliente)/buscar`                | Placeholder "Em breve"                                     | Mantido como placeholder visual (depende de endpoint `GET /barbearias` publico). `EmptyScreen` ja herda tokens novos                                                                                                                                                                                                                                                                                                                                                                                                                              | ✅ Commit 9                                                                    |
| `/(cliente)/agendamentos`          | Placeholder "Em breve"                                     | Mantido como placeholder (depende de endpoint para role cliente). `EmptyScreen` herda tokens. **Refino futuro:** quando endpoint existir, tabs Proximos/Passados com underline ambar + CountdownTimer em cards futuros                                                                                                                                                                                                                                                                                                                            | ✅ Commit 9                                                                    |
| `/(cliente)/agendamentos/[codigo]` | Página de detalhe textual                                  | **Ticket premium**: hero `palette.surface` arredondado com `StatusBadge` size=md no topo, `TimeDisplay` XL (42pt JetBrains Mono âmbar) centralizado, sub-label "até HH:mm" em mono caption, data dia/mês/ano em Sora subheading, `CountdownTimer` quando agendamento futuro (verde/âmbar/vermelho pulsante <15min). Cards "Com quem" (avatar + nome), "Quando" (data + horário mono), "Serviços" (lista + total em mono). `DangerButton` "Cancelar agendamento" — `Alert.alert` confirmando. 7 testes preservados via `textTestID` no StatusBadge | ✅ Commit 8                                                                    |
| `/(cliente)/perfil`                | Re-export do perfil barbeiro                               | Re-export preservado (DRY) — herda toda a migração feita no perfil barbeiro                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | ✅ Commit 9                                                                    |
...
| Fase                             | Suites | Tests   |
| -------------------------------- | ------ | ------- |
| Antes (mobile/base)              | 44     | ~210    |
| Após Fase 1 (DS base)            | 62     | 282     |
| Após Fase 2 (telas redesenhadas) | 65     | 303     |
| Após Fase 3 (tab bars)           | 66     | 308     |
| Final (limpeza Button)           | 65     | **302** |

Líquido: **+18 testes** (helpers e componentes novos compensam os 6 testes do shim Button removidos).
