...
| 2FA setup                | `(barbeiro)/perfil/2fa.tsx` ← re-exportado por `(cliente)`          | 3 passos: intro hero `Feather shield` amber + benefits (`Feather key/clock/smartphone`), QR 160×160, 6 inputs 42×54 mono. Badge "2FA ativo" com `Feather check`. Ícones vetoriais Feather (`@expo/vector-icons`), sem emojis inline |
