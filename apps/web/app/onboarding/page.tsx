...
const DIAS_HORARIOS: HorarioData[] = [
  { key: "seg", label: "Segunda", aberto: true, abertura: "09:00", fechamento: "19:00" },
  { key: "ter", label: "Terça", aberto: true, abertura: "09:00", fechamento: "19:00" },
  { key: "qua", label: "Quarta", aberto: true, abertura: "09:00", fechamento: "19:00" },
  { key: "qui", label: "Quinta", aberto: true, abertura: "09:00", fechamento: "19:00" },
  { key: "sex", label: "Sexta", aberto: true, abertura: "09:00", fechamento: "19:00" },
  { key: "sab", label: "Sábado", aberto: true, abertura: "08:00", fechamento: "18:00" },
  { key: "dom", label: "Domingo", aberto: false, abertura: "09:00", fechamento: "18:00" },
];
...
function Passo1({
  ...
  function normalizeSlug(value: string) {
    onChange("slug", value.toLowerCase().replace(/[^a-z0-9-]/g, "").slice(0, 28));
  }
  ...
  return (
    <>
      ...
      <p className="ob-h1-sub">
        Esse nome aparece pra seus clientes no link de agendamento e nas mensagens automáticas.
      </p>
      ...
      <div className="ob-form">
        ...
        <div className="ob-field">
          ...
          <div className="with-prefix">
            ...
            <input type="text" value={data.slug} onChange={(event) => normalizeSlug(event.target.value)} />
          </div>
          <div className="help">
            <span style={{ color: slugDisponivel ? "var(--green)" : "var(--red)" }}>●</span>{" "}
            {slugDisponivel ? "Disponível" : "Escolha um slug maior"} · seus clientes vão ver{" "}
            <span style={{ color: "var(--accent)", fontFamily: "JetBrains Mono" }}>
...
function Passo2({
  ...
  return (
    <>
      ...
      <p className="ob-h1-sub">
        Escolha a cor principal. Ela vai pintar o painel, os botões e o link público dos seus clientes.
      </p>
      ...
      <div className="ob-form">
        ...
        <div className="ob-field">
          ...
          <div className="help">Opcional. Se não tiver, usamos a inicial do nome com sua cor.</div>
        </div>
        ...
        <div className="ob-field">
          ...
          <div className="brand-preview">
            ...
            <div>
              <div className="brand-preview-name">{barbearia.nome || "Sua Barbearia"}</div>
...
function Passo3({
  ...
  onChange: (index: number, key: keyof HorarioData, value: string | boolean) => void;
}) {
  return (
    <>
      <h1 className="ob-h1">Quando vocês abrem?</h1>
      <p className="ob-h1-sub">
        Define a janela em que os clientes podem agendar. Cada barbeiro pode ter horário próprio depois.
      </p>
        ...
        <div className="ob-field">
          ...
          <div className="hours-table">
            {horarios.map((horario, index) => (
              <div key={horario.key} className={`hour-row ${horario.aberto ? "" : "off"}`}>
                ...
                <input
                  ...
                  onChange={(event) => onChange(index, "abertura", event.target.value)}
                />
                <input
                  className="time-input"
                  type="time"
                  ...
                  disabled={!horario.aberto}
                  onChange={(event) => onChange(index, "fechamento", event.target.value)}
                />
                <Toggle checked={horario.aberto} onChange={(checked) => onChange(index, "aberto", checked)} />
              </div>
            ))}
          </div>
          <div className="help">
            Bloqueio de almoço e folgas individuais por barbeiro você configura depois, na agenda.
          </div>
        </div>
      </div>
    </>
...
function Passo4({
  ...
  onAddServico: () => void;
  onUpdateServico: (index: number, key: keyof ServicoData, value: string | number) => void;
  onRemoveServico: (index: number) => void;
}) {
  const presets: Array<{ id: ServicoPreset; label: string; desc: string; icon: string }> = [
  ...
  return (
    <>
      ...
      <p className="ob-h1-sub">
        Cadastre os serviços com preço e duração padrão. Cada barbeiro pode ter preço/duração próprios depois.
      </p>
          ...
          <div className="pick-grid pick-grid-3">
            {presets.map((item) => (
              ...
              >
                ...
                {preset === item.id && <span className="pick-check"><Check size={12} strokeWidth={3} /></span>}
              </button>
            ))}
          </div>
        </div>
          ...
          <div className="svc-list">
            {servicos.map((servico, index) => (
              <div key={index} className="svc-row">
                <input
                  value={servico.nome}
                  onChange={(event) => onUpdateServico(index, "nome", event.target.value)}
                  placeholder="Nome do serviço"
                />
                <input
                  value={servico.preco}
                  onChange={(event) => onUpdateServico(index, "preco", Number(event.target.value))}
                  placeholder="0"
                  type="number"
                  aria-label="Preço"
                />
                <input
                  ...
                  onChange={(event) => onUpdateServico(index, "duracao", Number(event.target.value))}
                  placeholder="30"
                  type="number"
                  aria-label="Duração"
                />
                <button type="button" className="del" aria-label="Remover serviço" onClick={() => onRemoveServico(index)}>
                  <X size={14} />
                </button>
              </div>
            ))}
...
function Passo5({
  ...
  onAddBarbeiro: () => void;
  onUpdateBarbeiro: (index: number, key: keyof BarbeiroData, value: string) => void;
  ...
  return (
    <>
      ...
      <p className="ob-h1-sub">
        Convide os barbeiros pelo e-mail. Eles recebem um link mágico, sem precisar de senha pra começar.
      </p>
      ...
      <div className="ob-form">
        <div className="ob-field">
          ...
          <div className="pick-grid pick-grid-2">
            <button type="button" className={`pick ${isBarbeiro ? "sel" : ""}`} onClick={() => setIsBarbeiro(true)}>
              <div className="pick-icon">●</div>
              <div className="pick-body">
                <div className="pick-title">Sim, atendo na cadeira</div>
                <div className="pick-desc">Você aparece na agenda como barbeiro</div>
              </div>
              {isBarbeiro && <span className="pick-check"><Check size={12} strokeWidth={3} /></span>}
            </button>
            <button type="button" className={`pick ${!isBarbeiro ? "sel" : ""}`} onClick={() => setIsBarbeiro(false)}>
              <div className="pick-icon">○</div>
              <div className="pick-body">
        ...
        <div className="ob-field">
          ...
          <div className="barber-list">
            {barbeiros.map((barbeiro, index) => (
              <div key={index} className="barber-row">
                <div className="barber-avatar">{(barbeiro.nome || "?").charAt(0).toUpperCase()}</div>
                <input
                  value={barbeiro.nome}
                  onChange={(event) => onUpdateBarbeiro(index, "nome", event.target.value)}
                  placeholder="Nome do barbeiro"
                />
                <input
                  value={barbeiro.email}
                  onChange={(event) => onUpdateBarbeiro(index, "email", event.target.value)}
                  placeholder="email@dominio.com"
                  type="email"
                />
                <button type="button" className="del" aria-label="Remover barbeiro" onClick={() => onRemoveBarbeiro(index)}>
                  <Trash2 size={14} />
                </button>
              </div>
            ))}
          ...
          <div className="help">
            Eles podem aceitar o convite depois. Você consegue avançar mesmo sem confirmação deles.
          </div>
        </div>
      </div>
    </>
...
function Passo6({
  ...
  return (
    <>
      ...
      <p className="ob-h1-sub">
        Revisa os dados. Você pode editar tudo depois, esse é só o ponto de partida.
      </p>

      <div className="summary-grid">
        <div className="summary-card">
          <h4>Barbearia <button type="button" className="edit" onClick={() => onGo(1)}>editar</button></h4>
          <div className="summary-line"><span className="k">Nome</span><span className="v">{barbearia.nome || "-"}</span></div>
          <div className="summary-line"><span className="k">Cidade</span><span className="v">{barbearia.cidade || "-"}</span></div>
          <div className="summary-line"><span className="k">Link</span><span className="v">toqe.app/{barbearia.slug || "-"}</span></div>
        </div>

        <div className="summary-card">
          <h4>Identidade <button type="button" className="edit" onClick={() => onGo(2)}>editar</button></h4>
          <div className="summary-line">
            <span className="k">Cor de marca</span>
            <span className="v color-summary"><span style={{ background: branding.cor }} />{branding.cor}</span>
          </div>
          <div className="summary-line"><span className="k">Logotipo</span><span className="v muted">não enviado · usaremos inicial</span></div>
        </div>

        <div className="summary-card">
          <h4>Funcionamento <button type="button" className="edit" onClick={() => onGo(3)}>editar</button></h4>
          {diasAbertos.slice(0, 3).map((horario) => (
            <div key={horario.key} className="summary-line">
              <span className="k">{horario.label}</span>
              <span className="v">{horario.abertura} - {horario.fechamento}</span>
            </div>
          ))}
        </div>

        <div className="summary-card">
          <h4>Serviços e equipe <button type="button" className="edit" onClick={() => onGo(4)}>editar</button></h4>
          <div className="summary-line"><span className="k">Serviços</span><span className="v">{servicos.length} cadastrados</span></div>
          <div className="summary-line"><span className="k">Barbeiros</span><span className="v">{barbeiros.length} convidados</span></div>
          <div className="summary-line"><span className="k">Plano</span><span className="v accent">Free · 30 dias do Basic grátis</span></div>
        </div>
      </div>

      <div className="ready-banner">
        <div className="ico"><Check size={22} strokeWidth={3} /></div>
        <div>
          <h4>Tudo certo pra ir ao ar</h4>
          <p>
            Ao publicar, seu link <span>toqe.app/{barbearia.slug || "urban"}</span> fica acessível e seus clientes já podem agendar.
          </p>
        </div>
      </div>
    </>
...
export default function Onboarding() {
  ...
  const [servicos, setServicos] = useState<ServicoData[]>(PRESETS_SERVICOS.basic);
  ...
  function updateHorario(index: number, key: keyof HorarioData, value: string | boolean) {
    setHorarios((prev) => prev.map((horario, itemIndex) => (itemIndex === index ? { ...horario, [key]: value } : horario)));
  }
  ...
  function updateServico(index: number, key: keyof ServicoData, value: string | number) {
    setServicos((prev) => prev.map((servico, itemIndex) => (itemIndex === index ? { ...servico, [key]: value } : servico)));
  }

  function updateBarbeiro(index: number, key: keyof BarbeiroData, value: string) {
  ...
  return (
    <div className="tqe-onboarding ob-shell" data-screen-label="01 Onboarding">
      <aside className="ob-side">
        <button type="button" className="ob-logo" onClick={() => router.push("/")}>
          <div className="ob-logo-mark"><Scissors size={14} strokeWidth={2.5} /></div>
          <span>Toqe</span>
        </button>

        <div>
          <h2 className="ob-side-title">Vamos botar sua barbearia<br />no ar.</h2>
          <p className="ob-side-sub">6 passos rápidos. Você pode pular o que quiser e configurar depois.</p>
        </div>
        ...
        <div className="ob-progress">
          {STEPS.map((item) => (
            <button
              ...
              onClick={() => go(item.num)}
            >
              <div className="num">{step > item.num ? <Check size={13} strokeWidth={3} /> : <span>{item.num}</span>}</div>
              <div className="info">
        ...
        <div className="ob-footer-help">
          ...
          <p>Time de migração disponível das 8h às 22h. Trazemos seus clientes do caderno pra cá em até 24h.</p>
          <a href="https://wa.me/" target="_blank" rel="noreferrer">Falar no WhatsApp <ChevronRight size={12} /></a>
        </div>
      </aside>
      ...
      <main className="ob-main">
        <AnimatePresence mode="wait">
          ...
          >
            <div className="ob-step-meta">
              <span className="tqe-ob-pill">{step === STEPS.length ? "FINAL" : `PASSO ${step} DE ${STEPS.length}`}</span>
            ...
            {step === 4 && (
              <Passo4
                ...
                onAddServico={() => setServicos((prev) => [...prev, { nome: "", preco: 0, duracao: 30 }])}
        ...
        <div className="ob-nav">
          <button type="button" className="tqe-ob-btn" disabled={step === 1} onClick={() => go(step - 1)}>
            ← Voltar
          </button>
