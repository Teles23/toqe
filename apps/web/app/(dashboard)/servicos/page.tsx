...
import {
  Plus, Scissors, Clock, DollarSign, X,
  Search,
  TrendingUp, Star, Edit2, Trash2,
} from "lucide-react";
...
const SERVICOS: Servico[] = [
  {
    id: 1, nome: "Corte Clássico", categoria: "corte",
    duracao: 30, preco: 30, ativo: true,
    descricao: "Corte tradicional com máquina e tesoura, acabamento com navalha.",
    pedidosMes: 48, receitaMes: 1440, avaliacao: 4.9,
    barbeiros: ["Carlos", "Lucas", "Felipe"],
  },
  {
    id: 2, nome: "Corte + Barba", categoria: "combo",
    duracao: 50, preco: 80, ativo: true,
    descricao: "Combinação completa de corte de cabelo com barba modelada.",
    pedidosMes: 32, receitaMes: 2560, avaliacao: 4.8,
    barbeiros: ["Carlos", "Lucas"],
  },
  {
    id: 3, nome: "Barba Completa", categoria: "barba",
    duracao: 30, preco: 40, ativo: true,
    descricao: "Modelagem de barba com toalha quente e navalha profissional.",
    pedidosMes: 21, receitaMes: 840, avaliacao: 4.7,
    barbeiros: ["Carlos", "Felipe"],
  },
  {
    id: 4, nome: "Degradê", categoria: "corte",
    duracao: 40, preco: 45, ativo: true,
    descricao: "Corte degradê com transição suave, acabamento preciso.",
    pedidosMes: 18, receitaMes: 810, avaliacao: 4.9,
    barbeiros: ["Lucas"],
  },
  {
    id: 5, nome: "Sobrancelha", categoria: "estetica",
    duracao: 15, preco: 20, ativo: true,
    descricao: "Design e modelagem de sobrancelha masculina.",
    pedidosMes: 15, receitaMes: 300, avaliacao: 4.6,
    barbeiros: ["Felipe"],
  },
  {
    id: 6, nome: "Pigmentação", categoria: "tratamento",
    duracao: 60, preco: 90, ativo: true,
    descricao: "Pigmentação capilar para cobertura de falhas e entradas.",
    pedidosMes: 8, receitaMes: 720, avaliacao: 4.8,
    barbeiros: ["Lucas"],
  },
  {
    id: 7, nome: "Hidratação", categoria: "tratamento",
    duracao: 45, preco: 60, ativo: false,
    descricao: "Tratamento de hidratação profunda para cabelos ressecados.",
    pedidosMes: 0, receitaMes: 0, avaliacao: 0,
    barbeiros: ["Felipe"],
  },
  {
    id: 8, nome: "Corte Infantil", categoria: "corte",
    duracao: 25, preco: 25, ativo: true,
    descricao: "Corte especial para crianças até 12 anos, com paciência e cuidado.",
    pedidosMes: 12, receitaMes: 300, avaliacao: 5.0,
    barbeiros: ["Carlos", "Felipe"],
  },
];

const CATEGORIA_CONFIG: Record<Categoria, { label: string; color: string; bg: string }> = {
  corte:      { label: "Corte",      color: "var(--status-info)",    bg: "rgba(77,163,255,0.1)"  },
  barba:      { label: "Barba",      color: "var(--status-success)", bg: "rgba(29,185,84,0.1)"   },
  combo:      { label: "Combo",      color: "var(--status-warning)", bg: "rgba(244,180,0,0.1)"   },
  estetica:   { label: "Estética",   color: "#C084FC",               bg: "rgba(192,132,252,0.1)" },
  tratamento: { label: "Tratamento", color: "var(--status-error)",   bg: "rgba(255,77,79,0.1)"   },
};

/* ── Modal de novo/editar serviço ── */
function ServicoModal({ servico, onClose }: { servico?: Servico; onClose: () => void }): React.JSX.Element {
          ...
          <div className="px-5 py-4 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input
                  type="number"
                  value={preco}
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => setPreco(Number(e.target.value))}
                  className="tqe-input"
                  min={0}
                />
              </div>
              <div>
                ...
                <input
                  type="number"
                  value={duracao}
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => setDuracao(Number(e.target.value))}
                  className="tqe-input"
                  min={5}
                  step={5}
                />
            ...
            <div>
              ...
              <textarea
                ...
                onFocus={(e: React.FocusEvent<HTMLTextAreaElement>) => (e.target.style.borderColor = "var(--status-info)")}
                onBlur={(e: React.FocusEvent<HTMLTextAreaElement>)  => (e.target.style.borderColor = "var(--border-default)")}
              />
            </div>
          </div>
...
/* ── Painel de detalhe ── */
function ServicoDetalhe({ s, onClose, onEdit }: { s: Servico; onClose: () => void; onEdit: () => void }): React.JSX.Element {
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="px-4 py-5" style={{ borderBottom: "1px solid var(--border-subtle)" }}>
          ...
          <div className="flex items-center gap-2 mb-3">
            ...
            <span
              className="text-[10px] font-semibold px-2 py-0.5 rounded"
              style={{
                background: s.ativo ? "rgba(29,185,84,0.1)" : "rgba(255,255,255,0.04)",
        ...
        <div
          ...
          style={{ borderBottom: "1px solid var(--border-subtle)", borderColor: "var(--border-subtle)" }}
        >
          {[
        ...
        <div className="grid grid-cols-2 gap-2 px-4 py-4" style={{ borderBottom: "1px solid var(--border-subtle)" }}>
          {[
            ...
            { label: "Receita/mês",   value: s.receitaMes > 0 ? `R$${s.receitaMes}` : "—" },
            { label: "Avaliação",     value: s.avaliacao > 0 ? `${s.avaliacao} ★` : "—" },
        ...
        <div className="px-4 py-3">
          <span className="block text-[10px] uppercase tracking-wider mb-2" style={{ color: "var(--text-muted)" }}>
  ...
  return (
    <div className="max-w-7xl mx-auto flex flex-col gap-5 h-full">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          { label: "Serviços ativos", value: totalAtivos,  status: "info"    as const, icon: Scissors    },
          { label: "Pedidos este mês", value: totalPedidos, status: "success" as const, icon: TrendingUp   },
          { label: "Receita do mês",  value: totalReceita, unit: "R$", status: "warning" as const, icon: DollarSign },
          { label: "Ticket médio",    value: ticketMedio,  unit: "R$", status: "neutral" as const, icon: Star       },
        ].map((card, i) => (
          <motion.div
            key={card.label}
          ...
          >
            ...
            <div className="relative flex items-center" style={{ minWidth: 160 }}>
              ...
              <input
                ...
                style={{
                  paddingLeft: 28, paddingRight: 12, height: 32,
                  background: "var(--bg-secondary)", border: "1px solid var(--border-default)",
            ...
            <div className="flex gap-1 flex-wrap">
              <button
                ...
                style={{
                  background: filterCat === "todos" ? "var(--bg-hover)" : "transparent",
                  border: `1px solid ${filterCat === "todos" ? "var(--border-strong)" : "transparent"}`,
                  color: filterCat === "todos" ? "var(--text-primary)" : "var(--text-muted)",
                }}
              >
                Todos
              </button>
            ...
            >
              {(["todos", "ativo", "inativo"] as const).map(f => (
                <button
                  ...
                  style={{
                    background: filterAtivo === f ? "var(--bg-card)" : "transparent",
                    color: filterAtivo === f ? "var(--text-primary)" : "var(--text-muted)",
                ...
                {filtered.map((s, i) => {
                  ...
                  return (
                    <motion.div
                      ...
                      style={{
                        background: isSelected ? `${cfg.color}06` : "var(--bg-secondary)",
