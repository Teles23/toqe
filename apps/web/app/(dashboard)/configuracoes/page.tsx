...
import {
  Store, Clock, Bell, CreditCard, Shield,
  Check, Zap,
...
function ConfigRow({
  label, desc, children,
}: { label: string; desc?: string; children: React.ReactNode }): React.JSX.Element {
  return (
    <div
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {[
          { label: "Nome da barbearia", icon: Store,  value: nome,  set: setNome  },
...
function SecaoHorarios(): React.JSX.Element {
  ...
  function toggle(idx: number) {
    setHorarios(prev => prev.map((h, i) => i === idx ? { ...h, aberto: !h.aberto } : h));
  }
      ...
      >
        {horarios.map((h, i) => (
          <div
            ...
            style={{
              borderBottom: i < horarios.length - 1 ? "1px solid var(--border-subtle)" : "none",
            ...
            {h.aberto ? (
              <div className="flex items-center gap-2 flex-1">
                <input
                  type="time"
                  value={h.abertura}
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => setHorarios(prev => prev.map((x, j) => j === i ? { ...x, abertura: e.target.value } : x))}
                  style={{
                ...
                <input
                  type="time"
                  value={h.fechamento}
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => setHorarios(prev => prev.map((x, j) => j === i ? { ...x, fechamento: e.target.value } : x))}
                  style={{
                    height: 30, padding: "0 8px",
                    background: "var(--bg-secondary)", border: "1px solid var(--border-default)",
                    borderRadius: 6, color: "var(--text-primary)", fontSize: 12,
              ...
              <span className="text-[12px] flex-1" style={{ color: "var(--text-muted)" }}>Fechado</span>
            )}
          </div>
        ))}
      </div>
  ...
  const grupos = [
    {
      titulo: "Agendamentos",
      items: [
        { key: "novoAgendamento"  as const, label: "Novo agendamento",     desc: "Quando um cliente agenda pelo app" },
        { key: "cancelamento"     as const, label: "Cancelamento",          desc: "Quando um agendamento é cancelado" },
        { key: "lembreteCliente"  as const, label: "Lembrete para cliente", desc: "30min antes do horário agendado" },
        { key: "lembreteInternos" as const, label: "Lembrete interno",      desc: "Notifica a equipe antes do expediente" },
      ],
    },
    {
      titulo: "Operação",
      items: [
        { key: "relatorioDiario"   as const, label: "Relatório diário",    desc: "Resumo do dia enviado à noite" },
        { key: "clienteNovo"       as const, label: "Cliente novo",         desc: "Primeiro agendamento de um cliente" },
        { key: "avaliacaoRecebida" as const, label: "Avaliação recebida",   desc: "Quando um cliente avalia o serviço" },
        { key: "pagamentoRecebido" as const, label: "Pagamento confirmado", desc: "Confirmação de pagamento via app" },
      ],
    },
  ];
        ...
        >
          ...
          {grupo.items.map((item, i) => (
            <div
              ...
              style={{ borderBottom: i < grupo.items.length - 1 ? "1px solid var(--border-subtle)" : "none" }}
            >
              <div>
                <span className="block text-[13px] font-medium" style={{ color: "var(--text-primary)" }}>
                  {item.label}
              ...
              <Toggle checked={configs[item.key]} onChange={() => toggle(item.key)} />
            </div>
          ))}
        </div>
      ))}
...
function SecaoPlano(): React.JSX.Element {
  const planos = [
    {
      id: "basic", nome: "Basic", preco: 49,
      atual: false,
      features: ["1 barbearia", "Até 2 barbeiros", "50 agend./mês", "Suporte por e-mail"],
    },
    {
      id: "pro", nome: "Pro", preco: 99,
      atual: true,
      features: ["1 barbearia", "Até 10 barbeiros", "Agendamentos ilimitados", "WhatsApp + SMS", "Relatórios avançados", "Suporte prioritário"],
    },
    {
      id: "enterprise", nome: "Enterprise", preco: 249,
      atual: false,
      features: ["Múltiplas unidades", "Barbeiros ilimitados", "White-label", "API pública", "Gerente de conta dedicado"],
    },
  ];
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {planos.map(p => (
          <motion.div
            ...
            style={{
              background: p.atual ? "rgba(244,180,0,0.04)" : "var(--bg-secondary)",
          ...
          >
            ...
            <div
              ...
              style={{ height: 2, background: p.atual ? "var(--primary)" : "var(--border-default)", opacity: p.atual ? 0.8 : 0.3 }}
            />

            {p.atual && (
              <span
                className="inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded mb-3"
                style={{ background: "rgba(244,180,0,0.15)", color: "var(--primary)" }}
              >
            ...
            <div className="flex items-baseline gap-1 mb-4">
              <span
                ...
                style={{ fontFamily: "var(--font-heading)", color: p.atual ? "var(--primary)" : "var(--text-primary)", letterSpacing: "-0.04em" }}
              >
                R${p.preco}
              </span>
            ...
            <ul className="space-y-1.5 mb-4">
              {p.features.map(f => (
                <li key={f} className="flex items-center gap-2">
                  <Check size={11} style={{ color: p.atual ? "var(--primary)" : "var(--status-success)", flexShrink: 0 }} />
    ...
    <div className="space-y-6">
          ...
          {[
            ...
            <div key={f.key}>
              ...
              <input
                type="password"
                value={senha[f.key]}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => setSenha(prev => ({ ...prev, [f.key]: e.target.value }))}
                className="tqe-input"
                placeholder="••••••••"
              />
            </div>
      ...
      >
        ...
        <div className="px-4 py-3">
          <ConfigRow
            ...
            desc={twoFa ? "Ativado — via app autenticador" : "Adiciona uma camada extra de segurança"}
          >
      ...
      >
        ...
        >
          ...
          <button className="text-[11px] font-medium" style={{ color: "var(--status-error)" }}>
            Encerrar todas
          </button>
        </div>
        {sessoes.map((s, i) => (
          ...
          >
            <div className="flex items-center gap-3">
              <div
                ...
                style={{
                  width: 32, height: 32,
                  background: s.atual ? "rgba(29,185,84,0.1)" : "var(--bg-hover)",
                  color: s.atual ? "var(--status-success)" : "var(--text-muted)",
                }}
              >
                <Smartphone size={14} />
              </div>
            ...
            {!s.atual && (
              <button className="text-[11px] font-medium px-2.5 py-1 rounded-lg"
                style={{
        ...
        {SECOES.map((s) => {
          ...
          return (
            <button
              ...
              onMouseEnter={(e: React.MouseEvent<HTMLButtonElement>) => { if (!ativa) (e.currentTarget as HTMLElement).style.background = "var(--bg-hover)"; }}
              onMouseLeave={(e: React.MouseEvent<HTMLButtonElement>) => { if (!ativa) (e.currentTarget as HTMLElement).style.background = "transparent"; }}
            >
