...
const BARBEIROS: Barbeiro[] = [
  { id: 1, nome: "Carlos",  initial: "C", state: "active", agendamentos: 6, livres: 2 },
  { id: 2, nome: "Lucas",   initial: "L", state: "idle",   agendamentos: 4, livres: 4 },
  { id: 3, nome: "Felipe",  initial: "F", state: "active", agendamentos: 5, livres: 3 },
];

const SLOTS_INICIAL: Slot[] = [
  {
    id: 1, time: "08:30", endTime: "09:00",
    client: "João Silva",      clientInitial: "J",
    service: "Corte",          barbeiro: "Carlos",  barbeiroInitial: "C",
    duration: 30, status: "active", progressPct: 68, startedAt: "08:30",
  },
  {
    id: 2, time: "09:00", endTime: "09:50",
    client: "Pedro Santos",    clientInitial: "P",
    service: "Corte + Barba",  barbeiro: "Lucas",   barbeiroInitial: "L",
    duration: 50, status: "confirmed",
  },
  {
    id: 3, time: "09:30", endTime: "09:50",
    client: "Ana Costa",       clientInitial: "A",
    service: "Sobrancelha",    barbeiro: "Felipe",  barbeiroInitial: "F",
    duration: 20, status: "confirmed",
  },
  {
    id: 4, time: "10:00", endTime: "10:30",
    client: "Marco Oliveira",  clientInitial: "M",
    service: "Barba",          barbeiro: "Carlos",  barbeiroInitial: "C",
    duration: 30, status: "pending",
  },
  {
    id: 5, time: "10:30", endTime: "11:00",
    client: "Bruno Lima",      clientInitial: "B",
    service: "Corte",          barbeiro: "Lucas",   barbeiroInitial: "L",
    duration: 30, status: "late",
  },
  {
    id: 6, time: "12:00", endTime: "13:00",
    client: "Almoço",          clientInitial: "—",
    service: "Intervalo",      barbeiro: "Todos",   barbeiroInitial: "—",
    duration: 60, status: "blocked",
  },
  {
    id: 7, time: "13:00", endTime: "13:30",
    client: "—",               clientInitial: "—",
    service: "—",              barbeiro: "Carlos",  barbeiroInitial: "C",
    duration: 30, status: "available",
  },
  {
    id: 8, time: "14:00", endTime: "14:50",
    client: "Rafael Souza",    clientInitial: "R",
    service: "Corte + Barba",  barbeiro: "Felipe",  barbeiroInitial: "F",
    duration: 50, status: "confirmed",
  },
  {
    id: 9, time: "15:30", endTime: "16:00",
    client: "Diego Mendes",    clientInitial: "D",
    service: "Corte",          barbeiro: "Lucas",   barbeiroInitial: "L",
...
function StatusBadge({ status }: { status: SlotStatus }): React.JSX.Element {
  ...
  return (
    <span
      ...
      style={{
        background: cfg.bg !== "transparent" ? cfg.bg : "rgba(255,255,255,0.04)",
        color: cfg.color,
        border: `1px solid ${cfg.border}`,
      }}
    >
      <span style={{ animation: cfg.pulse ? "tqe-pulse-green 1.5s ease-in-out infinite" : "none" }}>
        {cfg.icon}
      </span>
      {cfg.label}
    </span>
...
function AgendaSlot({ slot, index }: { slot: Slot; index: number }): React.JSX.Element {
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-0.5">
            <span
              ...
              style={{
                color: isAvailable || isBlocked ? "var(--text-muted)" : "var(--text-primary)",
          ...
          {isActive && slot.progressPct !== undefined && (
            <div className="mt-2" style={{ maxWidth: 160 }}>
              <div
                ...
                style={{ height: 2, borderRadius: 2, background: "var(--border-default)" }}
              >
                <motion.div
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          animation: b.state === "active" ? "tqe-sidebar-pulse 2s ease-in-out infinite"
                   : b.state === "late"   ? "tqe-sidebar-pulse-red 1.5s ease-in-out infinite"
                   : "none",
        }}
      />
      ...
      <div className="flex items-center gap-3 pl-2">
        ...
        >
          ...
          <span
            ...
            style={{
              ...
              animation: b.state !== "idle" ? "tqe-pulse-green 2s ease-in-out infinite" : "none",
            }}
          />
        </div>
        ...
        >
          {b.state === "active" ? "Ativo" : b.state === "late" ? "Atrasado" : "Livre"}
        </span>
      </div>
    </motion.div>
  );
...
export default function Agenda(): React.JSX.Element {
  ...
  const filtered = slots.filter(s => {
    if (filterBarbeiro !== "Todos" && s.barbeiro !== filterBarbeiro) return false;
    if (filterStatus !== "Todos" && s.status !== filterStatus.toLowerCase()) return false;
    if (searchQuery && !s.client.toLowerCase().includes(searchQuery.toLowerCase())) return false;
    return true;
  });

  // Métricas rápidas ao vivo
  const totalHoje = slots.filter(s => !["blocked", "available"].includes(s.status)).length;
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-5">
          ...
          {days.map((d, i) => {
            ...
            return (
              <motion.button
                ...
                style={{
                  background: isSelected ? "var(--primary)" : isToday ? "var(--bg-hover)" : "var(--bg-card)",
                  border: `1px solid ${isSelected ? "var(--primary)" : "var(--border-default)"}`,
                  color: isSelected ? "#0D0D0D" : isToday ? "var(--text-primary)" : "var(--text-secondary)",
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: "Agendamentos",   value: totalHoje, color: "var(--status-info)"    },
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <div className="flex gap-1">
              {["Todos", ...BARBEIROS.map(b => b.nome)].map(name => (
                <button
                  ...
                  style={{
                    background: filterBarbeiro === name ? "var(--bg-hover)" : "transparent",
                    border: `1px solid ${filterBarbeiro === name ? "var(--border-strong)" : "transparent"}`,
                    color: filterBarbeiro === name ? "var(--text-primary)" : "var(--text-muted)",
                    fontFamily: "var(--font-body)",
                  }}
                >
                  {name}
            ...
            {[
              ...
              <button
                ...
                className="px-2.5 py-1 rounded text-[11px] font-medium transition-all"
                style={{
                  background: filterStatus === f.key ? "var(--bg-hover)" : "transparent",
                  border: `1px solid ${filterStatus === f.key ? "var(--border-strong)" : "transparent"}`,
                  color: filterStatus === f.key ? "var(--text-primary)" : "var(--text-muted)",
                  fontFamily: "var(--font-body)",
                }}
              >
                {f.label}
          ...
          >
            ...
            <div className="px-4 py-3 space-y-2.5">
              {[
                ...
                { label: "Tempo médio hoje",  value: "38min", color: "var(--status-info)"    },
              ].map(stat => (
