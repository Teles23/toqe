    ...
    >
      ...
      {liveStatus && (
        <motion.div
          ...
          style={{
            background: liveStatus.active ? "rgba(29,185,84,0.06)" : "rgba(255,255,255,0.04)",
        ...
        >
          <span
            ...
            style={{
              width: 5, height: 5,
              background: liveStatus.active ? "var(--status-success)" : "var(--text-muted)",
              animation: liveStatus.active ? "tqe-pulse-green 1.5s ease-in-out infinite" : "none",
            }}
          />
          <span
            className="text-[11px] font-medium"
            style={{ color: liveStatus.active ? "var(--status-success)" : "var(--text-secondary)" }}
          >
            {liveStatus.label}
          </span>
        </motion.div>
      ...
      <div className="relative hidden md:flex items-center">
        ...
        <input
          ...
          style={{
            ...
            height: 30, width: searchFocused ? 220 : 160,
            background: "var(--bg-card)",
            border: `1px solid ${searchFocused ? "var(--status-info)" : "var(--border-default)"}`,
            boxShadow: searchFocused ? "0 0 0 3px rgba(77,163,255,0.08)" : "none",
            borderRadius: 6,
      ...
      <div className="relative">
        <button
          ...
          style={{ color: notifOpen ? "var(--text-primary)" : "var(--text-secondary)" }}
        >
          <Bell size={16} />
          <span
              ...
              >
                ...
                >
                  ...
                  <span
                    ...
                    style={{ background: "rgba(255,77,79,0.12)", color: "var(--status-error)" }}
                  >
                    3 novas
                  </span>
                </div>
                {[
                  { title: "Cliente atrasado", desc: "João Silva — 08:30", time: "2min", color: "var(--status-error)" },
                  { title: "Agendamento confirmado", desc: "Pedro Santos — 09:00", time: "15min", color: "var(--status-success)" },
                  { title: "Horário livre encaixado", desc: "Carlos — 13:00", time: "1h", color: "var(--status-info)" },
                ].map((n, i) => (
                  <div
                    key={i}
