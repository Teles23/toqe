...
test.describe('Barbeiros', () => {
  test('listar → convidar → ver métricas', async ({ authenticatedPage: page }) => {
    await page.goto('/barbeiros');
    await expect(page.getByRole('heading', { name: /barbeiros/i })).toBeVisible();
    ...
    await page.goto('/barbeiros');
    const metricasBtn = page.getByRole('button', { name: /métricas|detalhes/i }).first();
