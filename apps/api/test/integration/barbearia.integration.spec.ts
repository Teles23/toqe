...
describe('Barbearia Integration', () => {
  ...
  beforeAll(async () => {
    ...
    await request(app.getHttpServer()).post('/auth/register').send({ nome: 'Dono', email, senha: 'Senha@123' });
    const loginRes = await request(app.getHttpServer()).post('/auth/login').send({ email, senha: 'Senha@123' });
    accessToken = loginRes.body.access_token;

    membroEmail = `membro_${Date.now()}@integration.com`;
    await request(app.getHttpServer()).post('/auth/register').send({ nome: 'Membro', email: membroEmail, senha: 'Senha@123' });
  });
