...
describe('UsuarioService', () => {
  ...
  describe('create', () => {
    it('cria usuário com sucesso', async () => {
      ...
      mockPrisma.usuario.create.mockResolvedValue({ codigo: 1, nome: 'Ana', email: 'ana@test.com' });

      const result = await service.create({ nome: 'Ana', email: 'ana@test.com', senha: 'abc123' });
  ...
  describe('me', () => {
    it('retorna perfil completo com barbearias', async () => {
      const user = { codigo: 1, nome: 'Ana', membros: [] };
      mockPrisma.usuario.findUnique.mockResolvedValue(user);
      const result = await service.me(1);
      expect(result).toHaveProperty('membros');
    });
  ...
  describe('update', () => {
    it('atualiza perfil do usuário', async () => {
      mockPrisma.usuario.findUnique.mockResolvedValue({ codigo: 1, nome: 'Ana', membros: [] });
      mockPrisma.usuario.update.mockResolvedValue({ codigo: 1, nome: 'Ana Nova' });
