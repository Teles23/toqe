import { Controller, Get, Param, Query, UseGuards, Headers, ParseIntPipe } from '@nestjs/common';
...
export class RelatorioController {
  ...
  @ApiQuery({ name: 'periodo', enum: ['7d', '30d', '3m', '6m', '12m'], required: false })
  ...
  @ApiResponse({ status: 200, description: 'Array de { data, concluido, cancelado, no_show }.' })
  agendamentos(
    @Param('barCodigo', ParseIntPipe) barCodigo: number,
    @Query('periodo') periodo: Periodo = '30d',
    @Headers('x-tenant-id') _tenantId: string,
  ...
  @ApiResponse({ status: 200, description: 'Array de { nome, quantidade, total }.' })
  servicos(
    @Param('barCodigo', ParseIntPipe) barCodigo: number,
    @Query('periodo') periodo: Periodo = '30d',
    @Headers('x-tenant-id') _tenantId: string,
  ...
  @ApiResponse({ status: 200, description: 'Array de 24 itens { hora, quantidade }.' })
  horariosPico(
    @Param('barCodigo', ParseIntPipe) barCodigo: number,
    @Query('periodo') periodo: Periodo = '30d',
    @Headers('x-tenant-id') _tenantId: string,
