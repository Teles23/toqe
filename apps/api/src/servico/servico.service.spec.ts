...
describe('ServicoService', () => {
  ...
  describe('create', () => {
    it('cria serviço com os dados corretos', async () => {
      ...
      expect(mockPrisma.servico.create).toHaveBeenCalledWith({ data: { ...dto, barCodigo } });
  ...
  describe('update', () => {
    it('atualiza e retorna serviço', async () => {
      const srv = { codigo: 5, nome: 'Barba', barCodigo };
      mockPrisma.servico.findFirst.mockResolvedValue(srv);
      mockPrisma.servico.update.mockResolvedValue({ ...srv, nome: 'Barba Premium' });

      const result = await service.update(5, { nome: 'Barba Premium' } as any, barCodigo);
